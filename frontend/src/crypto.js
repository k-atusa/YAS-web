...
class PQC1 {
    constructor() {
        ...
        Object.defineProperty(this, "pubKEM", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "priKEM", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "pubDSA", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "priDSA", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
    ...
    async loadkey(pub, pri) {
        if (pub) {
            const p = toU8(pub);
            if (p.length !== 4273)
                throw new Error("Invalid PQC1 public key (must be 4273 bytes)");
            this.pubX = p.slice(0, 56);
            this.pubEd = p.slice(56, 113);
        ...
        if (pri) {
            const p = toU8(pri);
            if (p.length !== 8177)
                throw new Error("Invalid PQC1 private key (must be 8177 bytes)");
            this.priX = p.slice(0, 56);
            this.priEd = p.slice(56, 113);
...
class Opsec {
    ...
    async encpub(method, publicBytes, privateBytes = null) {
        ...
        if (privateBytes !== null) {
            if (this.bodyKey.length > 0)
                ...
                this._sign = await am.sign(strToU8(this.msg));
            else {
    ...
    async decpub(privateBytes, publicBytes = null) {
        ...
        if (publicBytes !== null && this._sign.length > 0) {
            ...
            if (!ok)
                throw new Error(`${this._headAlgo.toUpperCase()} 서명이 유효하지 않습니다`);
        }
    }
}
...
export async function decryptOpsecPub(dataU8, myPrivateKey, peerPublicKey) {
    ...
    try {
        ...
        try {
            ...
            verified = false;
            verifyError = err.message || "서명 검증에 실패했습니다";
        }
        catch {
    ...
    const result = { msg: ops.msg, smsg: ops.smsg, files: [], verified, verifyError };
    if (ops.size >= 0) {
        const sm = new SymMaster(ops.bodyAlgo, ops.bodyKey);
        const decBody = await sm.deBin(dataU8.slice(reader.pos));
        if (ops.contAlgo === "zip1") {
